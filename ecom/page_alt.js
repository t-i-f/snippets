...
const handleSubmit = async (data, captcha) => {
  try {
    const response = await axios.post(seed.api, {data, captcha});
    ...
    <div className="h-full w-full p-4">
      ...
      >
        ...
        <div className="relative h-full w-full flex-none">
          <YouTubeEmbed videoid={seed.youtube} />
          <Lightbox
            index={selectedImage}
            slides={seed.slides}
        ...
        <div className="flex flex-col">
          <h1 className="text-2xl font-bold tracking-tight mt-2 sm:mt-0">{seed.name}</h1>
          <h2 className="sr-only">Product information</h2>
          <p className="text-xl font-medium tracking-tight">{seed.currency}{price}</p>
          <div className="mt-4">
          ...
          <Controller
            ...
            render={({ field: { onChange, onBlur, value } }) => (
              ...
              {seed.colors.map((color, index) => (
                <ColorRadioItem key={`${index}-${color.value}`} img={color.img} value={color.value} />
              ))}
              </RadioGroup>
            )}
          />
          <div className="mt-6 flex flex-col gap-1">
            <div className="mb-4 flex items-center gap-2 text-default-700">
              ...
              <p className="text-small font-medium">{seed.returns}</p>
            </div>
            <Controller
              name="size"
              control={control}
              render={({ field: { onChange, onBlur, value } }) => (
                ...
                {seed.sizes.map((size, index) => (
                  <TagGroupRadioItem key={`${index}-${size.value}`} value={size.value}>{size.value}</TagGroupRadioItem>
                ))}
                </RadioGroup>
              )}
            />
...
const SubmitButton = () => {
  ...
  >
    Order {color}, {size} for {seed.currency}{finalPrice}
  </Button>
}
...
export default function Home() {
  ...
  const methods = useForm({
    ...
    defaultValues: {
      access_key: seed.slug,
      color: seed.colors[0].value,
      size: seed.sizes[0].value,
      price: seed.price,
